...
class CollaborationServer implements \Ratchet\MessageComponentInterface {
    ...
    private function handleAIRequest($conn, $data) {
        ...
        $mappedAction = $actionMapping[$action] ?? 'explain';
        
        echo "🤖 AI請求: {$action} -> {$mappedAction}, 代碼長度: " . strlen($code) . "\n";
        ...
        try {
            // 調用後端 AI API
            $aiResponse = $this->callAIAPI($mappedAction, $code);
            ...
            echo "❌ AI API錯誤: " . $e->getMessage() . "\n";
            
            $conn->send(json_encode([
                'type' => 'ai_analysis_result',
                'success' => false,
                'error' => 'AI服務暫時不可用: ' . $e->getMessage(),
                'requestId' => $requestId
            ]));
        }
    }
    ...
    private function callAIAPI($action, $code) {
        ...
        // 修正 API 路徑 - 統一使用正確的後端路徑
        $apiUrl = 'http://localhost/PythonLearn-Zeabur-PHP-Notebook/backend/api/ai.php';
