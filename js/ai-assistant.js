...
class AIAssistantManager {
    ...
    handleAIResponse(response) {
        ...
        console.log('🔍 [AI Debug] 處理AI回應:', response);
        ...
        // 如果data是對象，提取message或analysis
        else if (response.data && (response.data.message || response.data.analysis)) {
            responseContent = response.data.message || response.data.analysis;
        }
        // 如果有suggestions數組，格式化顯示
        else if (response.data && response.data.suggestions && response.data.suggestions.length > 0) {
